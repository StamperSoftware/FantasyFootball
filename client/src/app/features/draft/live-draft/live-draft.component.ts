...
export class LiveDraftComponent implements OnDestroy {
    ...
    constructor () {
        ...
        this.leagueService.getLeague(this.leagueId).subscribe({
            next: async (league) => {
                ...
                this.userTeam.set(league.teams.find(ut => ut.userId == this.accountService.currentUser()?.id)!);
    ...
    async disconnect(){
        ...
        if (this.draftHelper()?.teams?.find(ut => ut.userId == this.accountService.currentUser()?.id)?.isOnline) {
